...
export default function AdminAnnouncementsPage() {
  return (
    <Phase9ResourcePage
      ...
      columns={[{ key: "title", label: "Judul" }, { key: "audience", label: "Audiens" }, { key: "status", label: "Status" }, { key: "publishedAt", label: "Publikasi", render: (row) => String(row.publishedAt ?? "-").slice(0, 10) }]}
      ...
      rowActions={[
        { label: "Publish", run: (api, row) => api.publishAnnouncement(row.id as string), show: (row) => row.status !== "PUBLISHED", variant: "soft" },
        { label: "Archive", run: (api, row) => api.archiveAnnouncement(row.id as string), show: (row) => row.status !== "ARCHIVED", variant: "outline" }
      ]}

...
type Overview = {
  ...
  finance: { invoicesIssued: number; verifiedPayments: number; outstandingInvoices: number; outstandingAmount: number; expensesThisMonth: number };
...
type QuickAlerts = {
  ...
  overdueInvoices: { count: number; items: Array<{ id: string; invoiceNumber: string; outstanding: number; student: { name: string; nis: string } }> };
...
const chartColors = ["#4f46e5", "#0ea5e9", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#64748b"];

const modules = [
  { title: "Siswa", description: "Data peserta didik aktif dan relasi kelas.", href: "/admin/students", icon: UsersRound, tone: "violet" as const },
  { title: "Guru", description: "Profil guru dan teaching assignment.", href: "/admin/teachers", icon: GraduationCap, tone: "blue" as const },
  { title: "Akademik", description: "Jadwal, presensi, nilai, dan mapel.", href: "/admin/academic/teaching-assignments", icon: BookOpenCheck, tone: "cyan" as const },
  { title: "Keuangan", description: "Invoice, pembayaran, dan pengeluaran.", href: "/admin/finance", icon: Landmark, tone: "emerald" as const },
  { title: "PPDB", description: "Pendaftaran dan seleksi peserta didik baru.", href: "/admin/ppdb", icon: Building2, tone: "amber" as const },
...
export default function AdminDashboardPage() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      setData({ overview, academic, finance, ppdb, people, activity, alerts, system });
  ...
  if (loading) return <LoadingState label="Memuat dashboard admin..." minHeight="min-h-[60vh]" />;
  if (error || !data) {
    return (
      <ErrorState
        action={<Button onClick={loadDashboard} variant="outline"><RefreshCcw className="h-4 w-4" /> Coba Lagi</Button>}
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={<UsersRound className="h-5 w-5" />} title="Siswa Aktif" tone="violet" value={formatNumber(data.overview.people.studentsActive)} />
        <StatCard icon={<GraduationCap className="h-5 w-5" />} title="Guru Aktif" tone="blue" value={formatNumber(data.overview.people.teachersActive)} />
        <StatCard icon={<UsersRound className="h-5 w-5" />} title="Staff" tone="emerald" value={formatNumber(data.overview.people.staffsActive)} />
        <StatCard icon={<Building2 className="h-5 w-5" />} title="Kelas" tone="amber" value={formatNumber(data.overview.academic.classrooms)} />
        <StatCard
        ...
        />
        <StatCard icon={<Building2 className="h-5 w-5" />} title="PPDB Aktif" tone="violet" value={formatNumber(data.overview.ppdb.activeRegistrations)} />
        <StatCard icon={<CalendarCheck className="h-5 w-5" />} title="Absensi Minggu Ini" tone="emerald" value={formatNumber(data.overview.academic.attendanceSessionsThisWeek)} />
        <StatCard icon={<BriefcaseBusiness className="h-5 w-5" />} title="Lowongan Aktif" tone="blue" value={formatNumber(data.overview.programs.publishedJobs)} />
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <ChartCard action={<Badge variant="info">Real API</Badge>} description="Distribusi pendaftaran berdasarkan status." title="PPDB Status Chart">
          <div className="h-72">
            <ResponsiveContainer height="100%" width="100%">
              <BarChart data={ppdbStatusData} margin={{ bottom: 0, left: -18, right: 8, top: 10 }}>
                ...
                <Bar dataKey="count" fill="#4f46e5" name="Pendaftar" radius={[10, 10, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </ChartCard>

        <ChartCard action={<Badge variant="success">Cashflow</Badge>} description="Pembayaran terverifikasi dibanding pengeluaran." title="Finance Monthly Cashflow">
          <div className="h-72">
            <ResponsiveContainer height="100%" width="100%">
        ...
        <ChartCard action={<Badge variant="secondary">Minggu Ini</Badge>} description="Rekap status presensi minggu berjalan." title="Attendance Overview">
          <div className="h-72">
            <ResponsiveContainer height="100%" width="100%">
              <PieChart>
                <Pie data={attendanceData} dataKey="count" innerRadius={58} nameKey="status" outerRadius={96} paddingAngle={3}>
                  {attendanceData.map((entry, index) => <Cell fill={chartColors[index % chartColors.length]} key={entry.status} />)}
                </Pie>
                <Tooltip contentStyle={tooltipStyle} />
              </PieChart>
            </ResponsiveContainer>
...
function QuickAlertsCard({ alerts }: { alerts: QuickAlerts }) {
  const rows = [
    { id: "overdue", label: "Invoice overdue", status: alerts.overdueInvoices.count > 0 ? "WARNING" : "OK", value: alerts.overdueInvoices.count },
    { id: "attendance", label: "Presensi tanpa record", status: alerts.attendanceMissing > 0 ? "WARNING" : "OK", value: alerts.attendanceMissing },
    { id: "payments", label: "Pembayaran ditolak", status: alerts.rejectedPayments > 0 ? "WARNING" : "OK", value: alerts.rejectedPayments },
    { id: "notifications", label: "Notifikasi unread", status: alerts.unreadNotifications > 0 ? "UNREAD" : "OK", value: alerts.unreadNotifications }
  ];
  ...
  return (
    <SectionCard description="Alert operasional yang perlu dipantau." title="Quick Alerts">
      <div className="space-y-3">
        {rows.map((row) => (
          <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-100 p-3" key={row.id}>
        ...
        {alerts.ppdbActive ? (
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-3 text-sm text-indigo-800">
            PPDB aktif: <span className="font-bold">{alerts.ppdbActive.name}</span>, berakhir {new Date(alerts.ppdbActive.endDate).toLocaleDateString("id-ID")}.
          </div>
        ) : null}
      </div>
    </SectionCard>
  ...
  return (
    <SectionCard description="Audit log terbaru dari sistem." title="Recent Activity">
      {activity.length === 0 ? (
        ...
        <DataTable
          columns={[
            { header: "Action", key: "action", cell: (row) => <span className="font-bold text-slate-900">{row.action}</span> },
    ...
    <SectionCard description="Status layanan runtime dan indikator sistem." title="System Status">
      <ul className="space-y-3">
        {rows.map((row) => {
          const Icon = row.icon;
          return (
            <li className="flex items-center justify-between rounded-2xl border border-slate-100 p-3" key={row.label}>
              <div className="flex items-center gap-3">
      ...
      <Button asChild className="mt-4 w-full" variant="soft">
        <Link href="/admin/reports"><BarChart3 className="h-4 w-4" /> Lihat Laporan</Link>
      </Button>
    </SectionCard>
  );
}

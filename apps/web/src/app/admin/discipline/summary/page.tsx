  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SectionCard description="Masukkan Student ID untuk melihat detail siswa." title="Ringkasan Siswa">
          <form className="flex flex-col gap-3 sm:flex-row" onSubmit={loadStudent}>
            <Input onChange={(event) => setStudentId(event.target.value)} placeholder="Student ID" value={studentId} />
            <Button disabled={loading} type="submit"><Search className="h-4 w-4" /> Cari</Button>
            <Button disabled={!studentSummary} onClick={printStudentSummary} type="button" variant="outline"><FileDown className="h-4 w-4" /> Print</Button>
          </form>
        </SectionCard>
        <SectionCard description="Masukkan Classroom ID untuk rekap per siswa." title="Ringkasan Kelas">
          <form className="flex flex-col gap-3 sm:flex-row" onSubmit={loadClassroom}>
            <Input onChange={(event) => setClassroomId(event.target.value)} placeholder="Classroom ID" value={classroomId} />
            <Button disabled={loading} type="submit"><Search className="h-4 w-4" /> Cari</Button>
          </form>
        </SectionCard>
      </div>
      ...
      {loading ? (
        <div className="grid min-h-40 place-items-center rounded-3xl bg-white shadow-soft">

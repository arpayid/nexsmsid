  ...
  return (
    <Phase9ResourcePage
      ...
      fields={[
        ...
        { label: "Rule ID", name: "ruleId", required: true },
      ...
      rowActions={[
        { label: "Confirm", run: (api, row) => api.confirmDisciplineViolation(row.id as string), show: (row) => row.status === "DRAFT", variant: "primary" },
        { label: "Cancel", run: (api, row) => api.cancelDisciplineViolation(row.id as string), show: (row) => row.status !== "CANCELLED", variant: "outline" },
        {
          label: "Print",
          run: async (api, row) => {

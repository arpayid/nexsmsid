...
export default function BkkJobsPage() {
  return (
    <Phase9ResourcePage
      ...
      columns={[{ key: "title", label: "Lowongan" }, { key: "companyName", label: "Perusahaan" }, { key: "location", label: "Lokasi" }, { key: "employmentType", label: "Tipe" }, { key: "deadline", label: "Deadline", render: (row) => String(row.deadline ?? "-").slice(0, 10) }, { key: "status", label: "Status" }]}
      ...
      eyebrow="Phase 9 BKK"
      fields={[
        { name: "industryPartnerId", label: "ID Mitra Industri" },
      ...
      rowActions={[
        ...
        { label: "Close", run: (api, row) => api.closeJobVacancy(row.id as string), show: (row) => row.status === "PUBLISHED", variant: "outline" }
      ]}

...
export default function AlumniPage() {
  return (
    <Phase9ResourcePage
      ...
      columns={[{ key: "name", label: "Nama" }, { key: "nis", label: "NIS" }, { key: "graduationYear", label: "Lulus" }, { key: "status", label: "Status" }, { key: "currentCompany", label: "Perusahaan" }, { key: "university", label: "Kampus" }]}
      create={(api, input) => api.createAlumni(input)}
      delete={(api, id) => api.deleteAlumni(id)}
      description="Kelola data alumni dan status karier."
      eyebrow="Phase 9 BKK"
      fields={[
        { name: "studentId", label: "ID Siswa" },
        { name: "nis", label: "NIS" },

...
export default function LettersArchivePage() {
  return (
    <Phase9ResourcePage
      ...
      rowActions={[
        { label: "Submit", run: (api, row) => api.submitLetter(row.id as string), show: (row) => row.status === "DRAFT", variant: "primary" },
        { label: "Issue", run: (api, row) => api.issueLetter(row.id as string), show: (row) => row.status === "DRAFT" || row.status === "SUBMITTED" || row.status === "APPROVED", variant: "primary" },
        { label: "Archive", run: (api, row) => api.archiveLetter(row.id as string), show: (row) => row.status === "ISSUED", variant: "outline" },
        { label: "Cancel", run: (api, row) => api.cancelLetter(row.id as string), show: (row) => !["ARCHIVED", "CANCELLED"].includes(String(row.status)), variant: "outline" },
        { label: "Reopen", run: (api, row) => api.reopenLetter(row.id as string), show: (row) => row.status === "REJECTED", variant: "soft" },
        {
          label: "PDF",
          run: async (api, row) => {

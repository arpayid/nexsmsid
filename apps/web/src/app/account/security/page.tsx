...
import { getStoredUser, clearAuthTokens } from "@/lib/auth-storage";
    ...
    <div className="space-y-8 pb-20">
      <div className="px-4 pt-6 sm:px-6 lg:px-8">
        <PageHeader 
          ...
          eyebrow="Phase 12.0 Account Security" 
          title="Keamanan Akun" 
        />
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Button onClick={handleLogoutAll} disabled={loggingOut} variant="outline" className="w-full text-rose-600 hover:bg-rose-50 hover:text-rose-700">
        ...
        <div className="mt-8">
          <Phase9ResourcePage
            ...
            columns={[
              ...
              { key: "userAgent", label: "Browser/Device", render: (row) => <span className="truncate max-w-[200px] block" title={String(row.userAgent ?? "-")}>{String(row.userAgent ?? "-")}</span> },
              { key: "success", label: "Status", render: (row) => <Badge variant={row.success ? 'success' : 'warning'}>{row.success ? 'Berhasil' : 'Gagal'}</Badge> },

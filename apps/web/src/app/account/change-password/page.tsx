  ...
  useEffect(() => {
    async function checkUser() {
      try {
        ...
        if ((user as any).forceChangePassword) {
          setForced(true);
        }
      } catch {
        // ignore
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (user) {
          router.replace(portalHomePath(resolvePortalForUser(user)));
        } else {
          router.replace("/");
        }
      }, 2000);
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-12 sm:px-6 lg:px-8">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-black">Ganti Password</CardTitle>
          <p className="mt-2 text-sm text-muted-foreground">
            {forced ? "Anda diwajibkan mengganti password untuk alasan keamanan sebelum melanjutkan." : "Ganti password akun Anda secara berkala untuk menjaga keamanan."}
          </p>
        </CardHeader>
        <CardContent>
          {success ? (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-emerald-800 text-center">
              <ShieldCheck className="mx-auto h-8 w-8 text-emerald-500 mb-2" />
              <p className="text-sm font-bold">Password berhasil diubah!</p>
              <p className="mt-1 text-xs">Mengarahkan ke dashboard...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-4">
                <label className="block space-y-1.5">
                  <span className="text-sm font-bold text-slate-700">Password Saat Ini</span>
                ...
                <label className="block space-y-1.5">
                  ...
                  <p className="text-xs text-slate-500">Gunakan kombinasi huruf besar, kecil, angka, dan simbol.</p>
                </label>

                <label className="block space-y-1.5">
                  <span className="text-sm font-bold text-slate-700">Konfirmasi Password Baru</span>

...
type PendingAssessment = { classroom: string | null; gradesCount: number; id: string; maxScore: number; name: string; subject: string | null; type: string };
...
export default function TeacherDashboardPage() {
  ...
  if (error || !dashboard) {
    return <ErrorState message={error ?? "Data dashboard tidak tersedia"} onRetry={loadData} title="Gagal memuat dashboard" />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard icon={<ClipboardCheck className="h-5 w-5" />} title="Absensi Pending" tone="amber" value={String(dashboard.counts.pendingAttendance)} />
        <StatCard icon={<GraduationCap className="h-5 w-5" />} title="Nilai Pending" tone="emerald" value={String(dashboard.counts.pendingGrades)} />
      </div>

      <SectionCard description="Aksi cepat untuk pekerjaan guru harian." title="Quick Action">
        <div className="grid gap-3 sm:grid-cols-3">
          <Button asChild variant="soft"><Link href="/teacher/attendance"><ClipboardCheck className="h-4 w-4" /> Input Absensi</Link></Button>
          <Button asChild variant="soft"><Link href="/teacher/grades"><GraduationCap className="h-4 w-4" /> Input Nilai</Link></Button>
          <Button asChild variant="outline"><Link href="/teacher/schedules"><CalendarDays className="h-4 w-4" /> Lihat Jadwal</Link></Button>
        </div>
      </SectionCard>
                ...
                <li className="rounded-2xl border border-slate-100 p-4" key={item.id}>
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{item.teachingAssignment?.classroom?.name ?? "Kelas"} - {item.room?.name ?? "Ruang"}</p>
                    </div>
            ...
            <ul className="divide-y divide-slate-100">
              {dashboard.pendingAttendance.slice(0, 5).map((session) => (
                <li className="flex items-center justify-between gap-3 py-3" key={session.id}>
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{session.schedule?.teachingAssignment?.subject?.name ?? "-"}</p>
                    <p className="text-xs text-muted-foreground">{session.schedule?.teachingAssignment?.classroom?.name ?? "-"} - {new Date(session.date).toLocaleDateString("id-ID")}</p>
                  </div>
            ...
            <ul className="divide-y divide-slate-100">
              {dashboard.pendingAssessments.slice(0, 5).map((assessment) => (
                <li className="flex items-center justify-between gap-3 py-3" key={assessment.id}>
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{assessment.name}</p>
                    <p className="text-xs text-muted-foreground">{assessment.subject ?? "-"} - {assessment.classroom ?? "-"}</p>
                  </div>

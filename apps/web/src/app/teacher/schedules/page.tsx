...
export default function TeacherSchedulesPage() {
  ...
  if (items.length === 0)
    return <EmptyState description="Belum ada jadwal mengajar untuk Anda." title="Belum ada jadwal" />;
                ...
                {grouped[day].map((s) => (
                  <li className="rounded-2xl border border-slate-100 p-3" key={s.id}>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {s.teachingAssignment?.classroom?.name ?? "-"} •{" "}
                      {s.teachingAssignment?.classroom?.competency?.name ?? "-"}
                    </p>
                  </li>
                ))}
              </ul>

...
export default function GuardianNotificationsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = (await api.getGuardianPortalNotifications({ limit: 50 })) as Notification[];
        if (!active) return;
        setItems(data);
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Portal Wali", "Notifikasi"]} description="Pemberitahuan untuk Anda" eyebrow="Portal Wali" title="Notifikasi" />

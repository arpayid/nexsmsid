...
export default function GuardianGradesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = (await api.getGuardianPortalChildGrades(childId as string)) as Grade[];
        if (!active) return;
        setItems(data);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : "Gagal memuat nilai");
      } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Portal Wali", "Nilai"]} description="Daftar nilai anak yang Anda pilih" eyebrow="Portal Wali" title="Nilai Anak" />
              ...
              <li className="flex flex-wrap items-center justify-between gap-3 py-3" key={g.id}>
                <div>
                  <p className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                    <GraduationCap className="h-4 w-4 text-primary" /> {g.assessment.teachingAssignment?.subject?.name ?? "-"} — {g.assessment.name}
                  </p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline">{g.score} / {g.assessment.maxScore}</Badge>
                </div>
              </li>
            ))}
          </ul>

...
export default function GuardianDisciplinePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = await api.getGuardianPortalChildDiscipline(childId as string) as Summary;
        if (active) setSummary(data);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : "Gagal memuat discipline summary anak");
      } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Portal Wali", "Kedisiplinan"]} description="Ringkasan poin pelanggaran confirmed dan prestasi anak yang dipilih." eyebrow="Portal Wali" title="Kedisiplinan Anak" />

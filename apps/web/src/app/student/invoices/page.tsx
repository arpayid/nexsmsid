...
export default function StudentInvoicesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = (await api.getStudentPortalInvoices()) as Invoice[];
        if (!active) return;
        setItems(data);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : "Gagal memuat tagihan");
      } finally {
      ...
      <SectionCard description={`${items.length} invoice`} title="Daftar Tagihan">
        <ul className="space-y-3">
          {items.map((inv) => {
            ...
            return (
              <li className="rounded-2xl border border-slate-100 p-4" key={inv.id}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <p className="mt-1 text-sm font-semibold text-slate-900">{formatRupiah(Number(inv.total))}</p>
                    {remaining > 0 ? (
                      <p className="text-xs text-rose-600">Sisa {formatRupiah(remaining)}</p>
                    ) : null}
                  </div>
                </div>
                {inv.items && inv.items.length > 0 ? (
                  <ul className="mt-3 space-y-1 border-t border-slate-100 pt-2 text-xs text-muted-foreground">
                    {inv.items.map((it) => (
                      <li className="flex items-center justify-between" key={it.id}>
                        <span>
                          {it.description} {it.paymentCategory?.name ? `(${it.paymentCategory.name})` : ""}
                        </span>
                        <span className="font-semibold text-slate-700">{formatRupiah(Number(it.amount))}</span>
                      </li>
                    ))}
                  </ul>
                ) : null}

...
export default function StudentAnnouncementsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = (await api.getStudentPortalAnnouncements({ limit: 30 })) as Announcement[];
        if (!active) return;
        setItems(data);
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Portal Siswa", "Pengumuman"]} description="Pengumuman resmi sekolah" eyebrow="Portal Siswa" title="Pengumuman" />

...
const DAY_LABEL: Record<string, string> = { MONDAY: "Senin", TUESDAY: "Selasa", WEDNESDAY: "Rabu", THURSDAY: "Kamis", FRIDAY: "Jumat", SATURDAY: "Sabtu", SUNDAY: "Minggu" };
...
export default function StudentSchedulesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (active) setError(err instanceof Error ? err.message : "Gagal memuat jadwal");
      } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Portal Siswa", "Jadwal"]} description="Jadwal pelajaran mingguan" eyebrow="Portal Siswa" title="Jadwal Pelajaran" />
          ...
          .map((day) => (
            <SectionCard key={day} title={DAY_LABEL[day] ?? day}>
              <ul className="space-y-3">
                {grouped[day].map((s) => (
                  <li className="rounded-2xl border border-slate-100 p-3" key={s.id}>
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-slate-900">{s.teachingAssignment?.subject?.name ?? "-"}</p>
                      <Badge variant="info">{s.startTime}–{s.endTime}</Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">{s.teachingAssignment?.teacher?.name ?? "-"}</p>
                    <p className="text-xs text-muted-foreground">{s.room?.name ?? "-"} • {s.lessonHour?.name ?? "-"}</p>
                  </li>
                ))}
              </ul>
            </SectionCard>

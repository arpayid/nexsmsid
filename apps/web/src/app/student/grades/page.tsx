...
export default function StudentGradesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = (await api.getStudentPortalGrades()) as Grade[];
        if (!active) return;
        setItems(data);
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Portal Siswa", "Nilai"]} description="Daftar nilai per mata pelajaran" eyebrow="Portal Siswa" title="Nilai Saya" />
            ...
            <ul className="divide-y divide-slate-100">
              {grades.map((g) => (
                <li className="flex flex-wrap items-center justify-between gap-3 py-3" key={g.id}>
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{g.assessment.name}</p>

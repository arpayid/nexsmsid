...
export function Phase9ResourcePage(props: Props) {
  ...
  const tableColumns = props.columns.map<DataTableColumn<Row>>((column) => ({
    cell: (row) => column.render ? column.render(row) : column.key === "status" ? <StatusBadge map={props.statusMap} value={row.status} /> : String(getPath(row, column.key) ?? "-"),
    header: column.label,
    key: column.key
  }));
  ...
  return (
    <div className="space-y-8">
      ...
      <SectionCard
        action={(
          <SearchFilterBar
            filters={props.statusOptions ? [{ label: "Status", onChange: setStatus, options: props.statusOptions, placeholder: "Semua status", value: status }] : undefined}
            onSearchChange={setSearch}
            onSubmit={handleSearch}
            searchValue={search}
          />
        )}
        description={<span>Total: <strong>{total}</strong> data</span>}
        title={props.title}
      >
        <DataTable
          actions={(row) => (
      ...
      <FormModal description="Lengkapi field yang tersedia." onClose={() => setFormOpen(false)} open={formOpen} title={editing ? "Edit Data" : "Tambah Data"}>
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          {(props.fields ?? []).map((field) => (
            <label className={field.type === "textarea" ? "space-y-2 md:col-span-2" : "space-y-2"} key={field.name}>
              ...
              {field.type === "select" ? (
                <select className="h-11 w-full rounded-xl border border-border bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" defaultValue={String(editing?.[field.name] ?? "")} name={field.name} required={field.required}>
                  <option value="">Pilih...</option>
                  {(field.options ?? []).map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>
              ) : field.type === "textarea" ? (
                <textarea className="min-h-24 w-full rounded-xl border border-border bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" defaultValue={String(editing?.[field.name] ?? "")} name={field.name} placeholder={field.placeholder} required={field.required} />
              ) : (

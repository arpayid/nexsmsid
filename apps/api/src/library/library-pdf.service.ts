...
export class LibraryPdfService {
  ...
  async generateLoanReceipt(id: string): Promise<Buffer> {
    ...
    const memberName = loan.member.student?.name || loan.member.teacher?.name || loan.member.staff?.name || loan.member.user?.name || loan.member.externalName || '-';
  ...
  async generateMemberCard(id: string): Promise<Buffer> {
    ...
    const memberName = member.student?.name || member.teacher?.name || member.staff?.name || member.user?.name || member.externalName || '-';

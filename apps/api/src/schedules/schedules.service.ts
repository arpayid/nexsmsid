...
export class SchedulesService {
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.validateConflicts(data, null);
    const item = await this.prisma.schedule.create({
      data,
      include: {
        teachingAssignment: { include: { teacher: true, subject: true, classroom: true } },
        room: true,
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "schedule.create", entity: "schedule", entityId: item.id, metadata: { teachingAssignmentId: data.teachingAssignmentId, roomId: data.roomId, dayOfWeek: data.dayOfWeek, lessonHourId: data.lessonHourId } });
    return item;
  }
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.validateConflicts(data as any, id);
    const item = await this.prisma.schedule.update({
      where: { id },
      data,
      include: { teachingAssignment: { include: { teacher: true, subject: true, classroom: true } }, room: true, lessonHour: true }
    });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "schedule.update", entity: "schedule", entityId: id, metadata: data });
    return item;
  }
  ...
  async delete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "schedule.delete", entity: "schedule", entityId: id, metadata: {} });
    return { id: item.id, deleted: true };
  }

  private async validateConflicts(data: { teachingAssignmentId: string; roomId: string; lessonHourId: string; dayOfWeek: string }, excludeId: string | null) {
    ...
    const teacherConflict = await this.prisma.schedule.findFirst({
      where: { deletedAt: null, ...whereExclude, dayOfWeek: data.dayOfWeek as any, lessonHourId: data.lessonHourId, teachingAssignment: { teacherId: ta.teacherId } },
      include: { teachingAssignment: { include: { teacher: true, subject: true } } }
    });
    if (teacherConflict) {
      throw new ConflictException(`Teacher ${ta.teacher.name} already has schedule for ${data.dayOfWeek} at this hour (${teacherConflict.teachingAssignment.subject.name})`);
    }

    const classroomConflict = await this.prisma.schedule.findFirst({
      where: { deletedAt: null, ...whereExclude, dayOfWeek: data.dayOfWeek as any, lessonHourId: data.lessonHourId, teachingAssignment: { classroomId: ta.classroomId } },
      include: { teachingAssignment: { include: { classroom: true, subject: true } } }
    });
    if (classroomConflict) {
      throw new ConflictException(`Classroom ${ta.classroom.name} already has schedule for ${data.dayOfWeek} at this hour (${classroomConflict.teachingAssignment.subject.name})`);
    }

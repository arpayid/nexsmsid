...
export class DashboardService {
  ...
  async getOverview() {
    ...
    const [
      ...
      this.prisma.student.count({ where: { deletedAt: null, status: "ACTIVE" as any } }),
      this.prisma.teacher.count({ where: { deletedAt: null, status: "ACTIVE" as any } }),
      this.prisma.staff.count({ where: { deletedAt: null, status: "ACTIVE" as any } }),
      ...
      this.prisma.payment.count({ where: { status: "VERIFIED" as any } }),
      this.prisma.invoice.findMany({
        where: { deletedAt: null, status: { in: ["ISSUED", "PARTIAL", "OVERDUE"] as any } },
        select: { total: true, paidAmount: true }
      }),
      this.prisma.expense.aggregate({
        where: { deletedAt: null, status: { in: ["APPROVED", "PAID"] as any }, date: { gte: startOfMonth, lte: endOfMonth } },
        _sum: { amount: true }
      }),
      this.prisma.ppdbPeriod.count({ where: { isActive: true } }),
      this.prisma.ppdbRegistration.count({
        where: { status: { in: ["SUBMITTED", "VERIFIED", "REVISION"] as any } }
      }),
      this.prisma.internship.count({ where: { status: "ONGOING" as any } }),
      this.prisma.jobVacancy.count({ where: { deletedAt: null, status: "PUBLISHED" as any } }),
      this.prisma.notification.count({ where: { status: "UNREAD" as any } })
    ]);
  ...
  async getFinanceSummary() {
    ...
    const [invoicesAggregate, verifiedPaymentsAggregate, expensesAggregate, outstandingInvoices, paymentsByMonth, expensesByMonth] =
      await Promise.all([
        ...
        this.prisma.expense.aggregate({
          where: { deletedAt: null, status: { in: ["APPROVED", "PAID"] as any } },
          _sum: { amount: true },
          _count: true
        }),
        this.prisma.invoice.findMany({
        ...
        this.prisma.payment.findMany({
          where: { status: "VERIFIED" as any, paidAt: { gte: startOfYear } },
          select: { amount: true, paidAt: true }
        }),
        this.prisma.expense.findMany({
          where: { deletedAt: null, status: { in: ["APPROVED", "PAID"] as any }, date: { gte: startOfYear } },
  ...
  async getPpdbSummary() {
    const [byStatus, byDepartment, byPeriod, totalRegistrations, activePeriods] = await Promise.all([
      ...
      this.prisma.ppdbRegistration.count(),
      this.prisma.ppdbPeriod.findMany({ where: { isActive: true }, select: { id: true, name: true, startDate: true, endDate: true, quota: true } })
    ]);
  ...
  async getQuickAlerts() {
    ...
    const [overdueInvoices, recentUnread, recentFailedPayments, currentPpdbOpen, lowQuota, todayAttendanceMissing] =
      await Promise.all([
        this.prisma.invoice.findMany({
          where: {
            deletedAt: null,
        ...
        this.prisma.notification.count({ where: { status: "UNREAD" as any } }),
        this.prisma.payment.count({ where: { status: "REJECTED" as any } }),
    ...
    return {
      ...
      ppdbActive: currentPpdbOpen
        ? { id: currentPpdbOpen.id, name: currentPpdbOpen.name, endDate: currentPpdbOpen.endDate }
        : null,

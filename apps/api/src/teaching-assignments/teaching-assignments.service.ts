...
export class TeachingAssignmentsService {
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    try {
      ...
      await this.auditService.record({ ...meta, actorId: actor.id, action: "teaching_assignment.create", entity: "teaching_assignment", entityId: item.id, metadata: { teacherId: data.teacherId, subjectId: data.subjectId, classroomId: data.classroomId } });
      return item;
    } catch (error) {
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    try {
      const item = await this.prisma.teachingAssignment.update({ where: { id }, data, include: { teacher: true, subject: true, classroom: true, academicYear: true, semester: true } });
      await this.auditService.record({ ...meta, actorId: actor.id, action: "teaching_assignment.update", entity: "teaching_assignment", entityId: id, metadata: data });
      return item;
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
        throw new ConflictException("Teaching assignment already exists for this teacher, subject, classroom, and semester");
  ...
  async delete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "teaching_assignment.delete", entity: "teaching_assignment", entityId: id, metadata: { teacherId: existing.teacherId, subjectId: existing.subjectId } });

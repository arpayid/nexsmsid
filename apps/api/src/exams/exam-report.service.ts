...
export class ExamReportService {
  ...
  async generateExamReport(examId: string, format: string, user: AuthenticatedUser, request: RequestWithUser): Promise<{ buffer: Buffer; contentType: string; filename: string }> {
    const exam = await this.prisma.exam.findFirst({
      where: { id: examId, deletedAt: null },
      include: {
        examType: true,
        academicYear: true,
        participants: { where: { deletedAt: null }, include: { student: { include: { user: { select: { id: true, name: true } } } } }, orderBy: { number: "asc" } }
      }
    });

...
export class PpdbRegistrationsService {
  ...
  async findById(id: string) {
    const item = await this.prisma.ppdbRegistration.findFirst({
      where: { id },
      include: {
        period: true, selectedDepartment: true, selectedCompetency: true,
        verifiedBy: true, convertedStudent: true,
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.registration.create", entity: "ppdb_registration", entityId: registration.id, metadata: { registrationNumber } });
    return registration;
  }

  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    const existing = await this.findById(id);
    if (!["DRAFT", "SUBMITTED", "REVISION"].includes(existing.status)) throw new BadRequestException("Cannot update registration in current status");
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.registration.update", entity: "ppdb_registration", entityId: id, metadata: data });
    return registration;
  }
  ...
  async changeStatus(id: string, newStatus: string, actor: AuthenticatedUser, meta: RequestMeta, extraData: Record<string, unknown> = {}) {
    ...
    const registration = await this.prisma.ppdbRegistration.update({
      where: { id },
      data: { status: newStatus as never, ...extraData, verifiedAt: newStatus === "VERIFIED" ? new Date() : existing.verifiedAt, verifiedById: newStatus === "VERIFIED" ? actor.id : existing.verifiedById },
      include: { period: true }
    });

    await this.prisma.ppdbStatusHistory.create({
      data: { registrationId: id, fromStatus: existing.status as never, toStatus: newStatus as never, changedById: actor.id, note: (extraData.note as string) || null }
    });
  ...
  async verify(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.verify", entity: "ppdb_registration", entityId: id, metadata: {} });
    return result;
  }
  ...
  async convertToStudent(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.convert", entity: "ppdb_registration", entityId: id, metadata: { studentId: student.id, nis } });
    return registration;
  }
  ...
  async createDocument(registrationId: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.document.create", entity: "ppdb_document", entityId: doc.id, metadata: { name: data.name } });
    return doc;
  }

  async updateDocument(documentId: string, input: unknown) {
    const parsed = input as Record<string, unknown>;
    return this.prisma.ppdbDocument.update({ where: { id: documentId }, data: parsed });
  }
  ...
  async verifyDocument(documentId: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.document.verify", entity: "ppdb_document", entityId: documentId, metadata: {} });
    return doc;
  }

  async rejectDocument(documentId: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    const parsed = input as Record<string, unknown>;
    const doc = await this.prisma.ppdbDocument.update({ where: { id: documentId }, data: { status: "REJECTED", note: (parsed.note as string) || null } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.document.reject", entity: "ppdb_document", entityId: documentId, metadata: {} });
    return doc;
  }
}

...
export class LettersService {
  ...
  async listTemplates(query: unknown) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.letterTemplate.findMany({ where, skip, take: params.limit, orderBy: [{ category: "asc" }, { code: "asc" }], include: this.templateInclude() }),
  ...
  async createTemplate(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.recordAudit("letters.template.create", "LetterTemplate", item.id, actor, meta, { templateId: item.id, category: item.category, code: item.code });
    return item;
  }
  ...
  async updateTemplate(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.recordAudit("letters.template.update", "LetterTemplate", item.id, actor, meta, { templateId: item.id, category: item.category, code: item.code });
    return item;
  }

  async deleteTemplate(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    const existing = await this.getTemplate(id);
    const item = await this.prisma.letterTemplate.update({ where: { id }, data: { deletedAt: new Date(), updatedById: actor.id }, include: this.templateInclude() });
    await this.recordAudit("letters.template.delete", "LetterTemplate", id, actor, meta, { templateId: id, category: existing.category, code: existing.code });
  ...
  async updateLetter(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    if (data.relatedCounselingCaseId !== undefined) updateData.relatedCounselingCase = data.relatedCounselingCaseId ? { connect: { id: data.relatedCounselingCaseId } } : { disconnect: true };
    if (data.relatedDisciplineViolationId !== undefined) updateData.relatedDisciplineViolation = data.relatedDisciplineViolationId ? { connect: { id: data.relatedDisciplineViolationId } } : { disconnect: true };
  ...
  async deleteLetter(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    const existing = await this.getLetter(id);
    if (!softDeletableStatuses.includes(existing.status)) throw new BadRequestException("Only DRAFT, REJECTED, or CANCELLED letters can be deleted");
    const item = await this.prisma.letter.update({ where: { id }, data: { deletedAt: new Date(), updatedById: actor.id }, include: this.letterInclude() });
  ...
  async submitLetter(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.$transaction(async (tx) => {
      ...
      return tx.letter.update({ where: { id }, data: { status: "SUBMITTED", submittedAt: new Date(), updatedById: actor.id }, include: this.letterInclude() });
    });
  ...
  async approveLetter(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.$transaction(async (tx) => {
      ...
      return tx.letter.update({
        where: { id },
        data: { status: "APPROVED", approvedAt: new Date(), approvedById: actor.id, rejectedAt: null, rejectedById: null, rejectionReason: null, updatedById: actor.id },
        include: this.letterInclude()
      });
    });
  ...
  async rejectLetter(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.$transaction(async (tx) => {
      ...
      return tx.letter.update({
        where: { id },
        data: { status: "REJECTED", rejectedAt: new Date(), rejectedById: actor.id, rejectionReason: data.rejectionReason, updatedById: actor.id },
        include: this.letterInclude()
      });
    });
  ...
  async issueLetter(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    const item = await this.prisma.$transaction(async (tx) => {
      ...
      if (["ARCHIVED", "CANCELLED"].includes(existing.status)) throw new BadRequestException("Archived or cancelled letters cannot be issued");
      if (existing.template?.requiresApproval && existing.status !== "APPROVED") throw new BadRequestException("This letter requires approval before issue");
      if (!existing.template?.requiresApproval && !["DRAFT", "SUBMITTED", "APPROVED"].includes(existing.status)) throw new BadRequestException("Letter cannot be issued from current status");
      const letterNumber = existing.letterNumber ?? await this.letterNumber.next(existing.category, new Date(), tx);
      return tx.letter.update({ where: { id }, data: { letterNumber, status: "ISSUED", issuedAt: new Date(), updatedById: actor.id }, include: this.letterInclude() });
    });
  ...
  async archiveLetter(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.letter.update({ where: { id }, data: { status: "ARCHIVED", archivedAt: new Date(), updatedById: actor.id }, include: this.letterInclude() });
  ...
  async cancelLetter(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    const existing = await this.getLetter(id);
    if (["ARCHIVED", "CANCELLED"].includes(existing.status)) throw new BadRequestException("Archived or cancelled letters cannot be cancelled");
    const item = await this.prisma.letter.update({ where: { id }, data: { status: "CANCELLED", cancelledAt: new Date(), updatedById: actor.id }, include: this.letterInclude() });
  ...
  async reopenLetter(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    const existing = await this.getLetter(id);
    if (existing.status === "CANCELLED" && !actor.roles.includes("super-admin")) throw new BadRequestException("Only Super Admin can reopen cancelled letters");
    if (!actor.roles.includes("super-admin") && existing.status !== "REJECTED") throw new BadRequestException("Only REJECTED letters can be reopened");
    if (!["REJECTED", "CANCELLED"].includes(existing.status)) throw new BadRequestException("Only REJECTED or CANCELLED letters can be reopened");
  ...
  async summary() {
    const [total, statusRows, recentIssued] = await Promise.all([
      ...
      this.prisma.letter.findMany({ where: { deletedAt: null, status: "ISSUED" }, take: 5, orderBy: { issuedAt: "desc" }, include: this.letterInclude() })
    ]);
  ...
  private async markApproval(tx: Prisma.TransactionClient, letterId: string, approverId: string, status: "APPROVED" | "REJECTED", note?: string) {
  ...
  private async assertLetterReferences(data: { templateId?: string; studentId?: string; guardianId?: string; teacherId?: string; staffId?: string; relatedCounselingCaseId?: string; relatedDisciplineViolationId?: string; recipientType?: string }) {
    const checks: Array<Promise<unknown>> = [];
    if (data.templateId) checks.push(this.assertExists(this.prisma.letterTemplate.count({ where: { id: data.templateId, deletedAt: null } }), "Letter template not found"));
    if (data.studentId) checks.push(this.assertExists(this.prisma.student.count({ where: { id: data.studentId, deletedAt: null } }), "Student not found"));
    if (data.guardianId) checks.push(this.assertExists(this.prisma.guardian.count({ where: { id: data.guardianId } }), "Guardian not found"));
    if (data.teacherId) checks.push(this.assertExists(this.prisma.teacher.count({ where: { id: data.teacherId, deletedAt: null } }), "Teacher not found"));
    if (data.staffId) checks.push(this.assertExists(this.prisma.staff.count({ where: { id: data.staffId, deletedAt: null } }), "Staff not found"));
    if (data.relatedCounselingCaseId) checks.push(this.assertExists(this.prisma.counselingCase.count({ where: { id: data.relatedCounselingCaseId, deletedAt: null } }), "Counseling case not found"));
    if (data.relatedDisciplineViolationId) checks.push(this.assertExists(this.prisma.disciplineViolation.count({ where: { id: data.relatedDisciplineViolationId, deletedAt: null } }), "Discipline violation not found"));
    ...
    if (data.recipientType === "GUARDIAN" && !data.guardianId) throw new BadRequestException("guardianId is required for GUARDIAN recipients");
  ...
  private letterAuditMetadata(letter: { id: string; letterNumber: string | null; status: string; category: string; recipientType: string; createdById: string; approvedById?: string | null }) {
    return {
      letterId: letter.id,
  ...
  private async recordAudit(action: string, entity: string, entityId: string, actor: AuthenticatedUser, meta: RequestMeta, metadata: Prisma.InputJsonValue) {

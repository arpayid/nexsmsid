...
export class InternshipLogsService {
  ...
  async findById(id: string) {
    const item = await this.prisma.internshipLog.findFirst({ where: { id }, include: { internship: { include: { student: true, industryPartner: true } }, reviewedBy: true } });
  ...
  async create(internshipId: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship-log.create", entity: "internship_log", entityId: item.id, metadata: { internshipId } });
    return item;
  }
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship-log.update", entity: "internship_log", entityId: id, metadata: data });
    return item;
  }

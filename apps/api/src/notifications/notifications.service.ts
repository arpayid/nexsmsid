...
export class NotificationsService {
  ...
  async markRead(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "notification.read", entity: "notification", entityId: id, metadata: {} });
    return item;
  }

  async markAllRead(actor: AuthenticatedUser, meta: RequestMeta) {
    const result = await this.prisma.notification.updateMany({ where: { userId: actor.id, status: "UNREAD" }, data: { status: "READ", readAt: new Date() } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "notification.read-all", entity: "notification", metadata: { count: result.count } });
  ...
  async archive(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "notification.archive", entity: "notification", entityId: id, metadata: {} });
    return item;
  }
}

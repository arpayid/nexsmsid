...
export type ResourceImportConfig = {
  ...
  transform?: (record: Record<string, unknown>, rowNumber: number, errors: ImportError[]) => Promise<Record<string, unknown>> | Record<string, unknown>;
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      try {
        ...
        const transformed = config.transform
          ? await config.transform(validation.data, rowNumber, errors)
          : validation.data;

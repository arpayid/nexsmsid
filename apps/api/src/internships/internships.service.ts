...
export class InternshipsService {
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship.create", entity: "internship", entityId: item.id, metadata: { title: item.title } });
    return item;
  }
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.internship.update({ where: { id }, data: this.cleanOptional(data), include: { student: true, industryPartner: true, supervisor: true } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship.update", entity: "internship", entityId: id, metadata: data });
    return item;
  }
  ...
  async delete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship.delete", entity: "internship", entityId: id, metadata: {} });
  ...
  async start(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.internship.update({ where: { id }, data: { status: "ONGOING" }, include: { student: true, industryPartner: true } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship.start", entity: "internship", entityId: id, metadata: {} });
    return item;
  }
  ...
  async complete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.internship.update({ where: { id }, data: { status: "COMPLETED", finalScore }, include: { student: true, industryPartner: true, score: true } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship.complete", entity: "internship", entityId: id, metadata: { finalScore } });
    return item;
  }
  ...
  async cancel(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.internship.update({ where: { id }, data: { status: "CANCELLED" }, include: { student: true, industryPartner: true } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship.cancel", entity: "internship", entityId: id, metadata: {} });
    return item;
  }
  ...
  async score(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "internship.score", entity: "internship", entityId: id, metadata: { finalScore } });
    return score;
  }

...
export class JobVacanciesService {
  ...
  async list(query: unknown) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.jobVacancy.findMany({ where, skip, take: params.limit, orderBy: { createdAt: "desc" }, include: { industryPartner: true, _count: { select: { applications: true } } } }),
  ...
  async publicList(query: unknown) {
    ...
    const skip = (params.page - 1) * params.limit;
    const [items, total] = await Promise.all([
      this.prisma.jobVacancy.findMany({ where, skip, take: params.limit, orderBy: { publishedAt: "desc" }, include: { industryPartner: true } }),
      this.prisma.jobVacancy.count({ where })
    ]);
    return { items, total, page: params.page, limit: params.limit };
  }
  ...
  async findById(id: string) {
    const item = await this.prisma.jobVacancy.findFirst({ where: { id, deletedAt: null }, include: { industryPartner: true, applications: true } });
    if (!item) throw new NotFoundException("Job vacancy not found");
    return item;
  }

  async publicFindById(id: string) {
    const item = await this.prisma.jobVacancy.findFirst({ where: { id, deletedAt: null, status: "PUBLISHED" }, include: { industryPartner: true } });
    if (!item) throw new NotFoundException("Job vacancy not found");
    return item;
  }
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.jobVacancy.create({ data: this.cleanOptional({ ...data, publishedAt: data.status === "PUBLISHED" ? new Date() : null }) as never, include: { industryPartner: true } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "job-vacancy.create", entity: "job_vacancy", entityId: item.id, metadata: { title: item.title } });
    return item;
  }
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "job-vacancy.update", entity: "job_vacancy", entityId: id, metadata: data });
    return item;
  }
  ...
  async delete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "job-vacancy.delete", entity: "job_vacancy", entityId: id, metadata: {} });
  ...
  async publish(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const item = await this.prisma.jobVacancy.update({ where: { id }, data: { status: "PUBLISHED", publishedAt: new Date() }, include: { industryPartner: true } });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "job-vacancy.publish", entity: "job_vacancy", entityId: id, metadata: {} });
    return item;
  }
  ...
  async close(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "job-vacancy.close", entity: "job_vacancy", entityId: id, metadata: {} });
    return item;
  }
  ...
  async publicApply(id: string, input: unknown) {
    ...
    try {
      return await this.prisma.jobApplication.create({ data: { jobVacancyId: id, ...this.cleanOptional(data), status: "SUBMITTED" } as never, include: { jobVacancy: true, alumni: true } });
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") throw new ConflictException("Applicant email has already applied for this job");
      throw error;
    }
  }
  ...
  private cleanOptional(data: Record<string, unknown>) {
    const cleaned = { ...data };
    for (const key of ["industryPartnerId", "qualification", "location", "employmentType", "salaryRange", "alumniId", "applicantEmail", "applicantPhone", "cvUrl", "note"]) {

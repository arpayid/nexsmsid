...
export class PpdbPeriodsService {
  ...
  async list(query: unknown) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.ppdbPeriod.findMany({
        where, skip, take: params.limit, orderBy: { startDate: "desc" },
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const period = await this.prisma.ppdbPeriod.create({
      data: { name: data.name, academicYearId: data.academicYearId || null, startDate: data.startDate, endDate: data.endDate, isActive: data.isActive, quota: data.quota || null },
      include: { academicYear: true }
    });
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.period.create", entity: "ppdb_period", entityId: period.id, metadata: { name: data.name } });
    return period;
  }
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.period.update", entity: "ppdb_period", entityId: id, metadata: data });
    return period;
  }
  ...
  async delete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "ppdb.period.delete", entity: "ppdb_period", entityId: id, metadata: {} });

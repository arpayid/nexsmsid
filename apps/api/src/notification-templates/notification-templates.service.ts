...
import { createNotificationTemplateSchema, notificationTemplateListQuerySchema, updateNotificationTemplateSchema } from "./notification-templates.dto";
...
export class NotificationTemplatesService {
  ...
  async list(query: unknown) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.notificationTemplate.findMany({ where, skip, take: params.limit, orderBy: { createdAt: "desc" }, include: { createdBy: { select: { id: true, email: true, name: true } } } }),
  ...
  async findById(id: string) {
    const item = await this.prisma.notificationTemplate.findFirst({ where: { id, deletedAt: null }, include: { createdBy: { select: { id: true, email: true, name: true } } } });
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    try {
      ...
      await this.auditService.record({ ...meta, actorId: actor.id, action: "notification-template.create", entity: "notification_template", entityId: item.id, metadata: { code: item.code } });
      return item;
    } catch (error) {
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    try {
      ...
      await this.auditService.record({ ...meta, actorId: actor.id, action: "notification-template.update", entity: "notification_template", entityId: id, metadata: data });
      return item;
    } catch (error) {
      this.handleUniqueError(error);
      throw error;
  ...
  async delete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "notification-template.delete", entity: "notification_template", entityId: id, metadata: {} });
  ...
  private handleUniqueError(error: unknown) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") throw new ConflictException("Notification template code already exists");
  }
}

...
export class HRService {
  ...
  async createPosition(data: CreateHRPositionDto, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.position.create", entity: "Created HR position", metadata: { positionId: created.id } });
    return created;
  }
  ...
  async getEmployees(params: { page?: number; limit?: number; search?: string; status?: string; employmentType?: string; positionId?: string }) {
  ...
  async getEmployee(id: string) {
    const employee = await this.prisma.employeeProfile.findFirst({
      ...
      include: { position: true, user: { select: { id: true, email: true, name: true } }, salaryComponents: { include: { component: true }, where: { deletedAt: null } } }
    });
  ...
  async createEmployee(data: CreateEmployeeProfileDto, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.employee.create", entity: "Created employee profile", metadata: { employeeId: created.id } });
    return created;
  }
  ...
  async updateEmployee(id: string, data: UpdateEmployeeProfileDto, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.employee.update", entity: "Updated employee profile", metadata: { employeeId: id } });
    return updated;
  }
  ...
  async deleteEmployee(id: string, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.employee.delete", entity: "Deleted employee profile", metadata: { employeeId: id } });
    return { success: true };
  }
  ...
  async createAttendance(data: CreateEmployeeAttendanceDto, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.attendance.create", entity: "Created employee attendance", metadata: { attendanceId: created.id } });
    return created;
  }
  ...
  async updateAttendance(id: string, data: UpdateEmployeeAttendanceDto, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.attendance.update", entity: "Updated employee attendance", metadata: { attendanceId: id } });
    return updated;
  }
  ...
  async deleteAttendance(id: string, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.attendance.delete", entity: "Deleted employee attendance", metadata: { attendanceId: id } });
    return { success: true };
  }
  ...
  async createLeaveRequest(data: CreateLeaveRequestDto, userId: string) {
    ...
    await this.audit.record({ actorId: userId, action: "hr.leave.create", entity: "Created leave request", metadata: { leaveId: created.id } });
  ...
  async cancelLeaveRequest(id: string, userId: string) {
    const leave = await this.getLeaveRequest(id);
    if (leave.status === 'REJECTED' || leave.status === 'CANCELLED') throw new BadRequestException('Leave request already rejected/cancelled');

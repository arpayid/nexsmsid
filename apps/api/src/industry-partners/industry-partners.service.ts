...
export class IndustryPartnersService {
  ...
  async list(query: unknown) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.industryPartner.findMany({ where, skip, take: params.limit, orderBy: { createdAt: "desc" }, include: { _count: { select: { internships: true, jobVacancies: true } } } }),
  ...
  async findById(id: string) {
    const item = await this.prisma.industryPartner.findFirst({ where: { id, deletedAt: null }, include: { internships: true, jobVacancies: true } });
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "industry-partner.create", entity: "industry_partner", entityId: item.id, metadata: { name: item.name } });
    return item;
  }
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "industry-partner.update", entity: "industry_partner", entityId: id, metadata: data });
    return item;
  }
  ...
  async delete(id: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "industry-partner.delete", entity: "industry_partner", entityId: id, metadata: {} });

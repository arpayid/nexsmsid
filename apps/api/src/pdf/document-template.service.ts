...
export class DocumentTemplateService {
  ...
  async renderPaymentReceiptPdf(paymentId: string): Promise<Buffer> {
    ...
    return this.pdf.render(header, (doc) => {
      ...
      this.pdf.drawSignatureBlock(doc, payment.invoice.student.classroom?.name || "Sekolah", "Petugas Keuangan", payment.verifiedBy?.name ?? null);
    });
  }
  ...
  async renderGradeRecapPdf(classroomId: string, semesterId?: string): Promise<Buffer> {
    ...
    const semester = semesterId
      ...
      : await this.prisma.semester.findFirst({ where: { isActive: true }, include: { academicYear: true }, orderBy: { startDate: "desc" } });

...
export class PdfService {
  ...
  drawKeyValueBlock(doc: PDFKit.PDFDocument, items: PdfKeyValue[], columns = 2) {
    ...
    const drawRow = (y: number) => {
      doc.moveTo(PAGE_MARGIN, y).lineTo(PAGE_WIDTH - PAGE_MARGIN, y).lineWidth(0.4).strokeColor("#ddd").stroke();
    };
  ...
  drawDivider(doc: PDFKit.PDFDocument) {
    const y = doc.y;
    doc.moveTo(PAGE_MARGIN, y).lineTo(PAGE_WIDTH - PAGE_MARGIN, y).lineWidth(0.5).strokeColor("#999").stroke();
    doc.moveDown(0.3);
  }

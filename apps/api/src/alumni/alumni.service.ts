...
export class AlumniService {
  ...
  async list(query: unknown) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.alumni.findMany({ where, skip, take: params.limit, orderBy: { createdAt: "desc" }, include: { student: true, _count: { select: { jobApplications: true, tracerStudies: true } } } }),
  ...
  async findById(id: string) {
    const item = await this.prisma.alumni.findFirst({ where: { id, deletedAt: null }, include: { student: true, jobApplications: true, tracerStudies: true } });
  ...
  async create(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    try {
      ...
      await this.auditService.record({ ...meta, actorId: actor.id, action: "alumni.create", entity: "alumni", entityId: item.id, metadata: { name: item.name } });
      return item;
    } catch (error) {
  ...
  async update(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    try {
      ...
      await this.auditService.record({ ...meta, actorId: actor.id, action: "alumni.update", entity: "alumni", entityId: id, metadata: data });
      return item;
    } catch (error) {
      this.handleUniqueError(error);
      throw error;
  ...
  async convertFromStudent(studentId: string, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    const [alumni] = await this.prisma.$transaction([
      existingAlumni
        ? this.prisma.alumni.update({ where: { id: existingAlumni.id }, data: { deletedAt: null, name: student.name, nis: student.nis, phone: student.phone, email: student.email, address: student.address, graduationYear, status: "ACTIVE" } })
        : this.prisma.alumni.create({ data: { studentId, nis: student.nis, name: student.name, graduationYear, phone: student.phone, email: student.email, address: student.address, status: "ACTIVE" } }),
      this.prisma.student.update({ where: { id: studentId }, data: { status: "GRADUATED" } })
    ]);

    await this.auditService.record({ ...meta, actorId: actor.id, action: "alumni.convert", entity: "alumni", entityId: alumni.id, metadata: { studentId } });
    return alumni;
  }

  private cleanOptional(data: Record<string, unknown>) {
    const cleaned = { ...data };
    for (const key of ["studentId", "nis", "phone", "email", "address", "currentCompany", "currentPosition", "university", "businessName"]) {

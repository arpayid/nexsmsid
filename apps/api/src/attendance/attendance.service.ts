...
export class AttendanceService {
  ...
  async listSessions(query: unknown) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.attendanceSession.findMany({
        where, skip, take: params.limit, orderBy: { date: "desc" },
        include: {
          schedule: {
            include: {
  ...
  async createSession(input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "attendance.session.create", entity: "attendance_session", entityId: item.id, metadata: { scheduleId: data.scheduleId, date: data.date.toISOString() } });
    return item;
  }
  ...
  async updateSession(id: string, input: unknown, actor: AuthenticatedUser, meta: RequestMeta) {
    ...
    await this.auditService.record({ ...meta, actorId: actor.id, action: "attendance.session.update", entity: "attendance_session", entityId: id, metadata: data });
    return item;
  }
  ...
  async getStudentSummary(studentId: string) {
    ...
    const records = await this.prisma.attendanceRecord.findMany({
      where: { studentId },
      include: { session: { include: { schedule: { include: { teachingAssignment: { include: { subject: true } }, lessonHour: true } } } } },

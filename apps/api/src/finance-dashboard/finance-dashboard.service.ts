...
export class FinanceDashboardService {
  ...
  async summary() {
    const [invoices, verifiedPayments, pendingPayments, expenses, outstanding] = await Promise.all([
      ...
      this.prisma.invoice.findMany({ where: { deletedAt: null, status: { in: ["ISSUED", "PARTIAL"] } }, select: { total: true, paidAmount: true }, take: 100 })
    ]);
  ...
  async outstanding() {
    const invoices = await this.prisma.invoice.findMany({
      ...
      select: { id: true, invoiceNumber: true, total: true, paidAmount: true, dueDate: true, status: true, student: { select: { name: true, nis: true } } },
      orderBy: { dueDate: "asc" },
      take: 50
    });
